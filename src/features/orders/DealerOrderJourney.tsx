...
export function DealerOrderJourney({ product, requestOrderOtp, onBack }: { product: CatalogueProduct; requestOrderOtp: (purpose: "ORDER_SUBMISSION") => Promise<string>; onBack?: () => void }) {
  ...
  return <main className="commerce-page commerce-pdp">
    ...
    <section className={`commerce-order-tray ${totalPairs > 0 ? "is-active" : ""}`} aria-label="Current Order action"><div><span>{totalPairs} pairs selected</span><strong>{draft ? formatRetailValue(draft.retailValueMinor, draft.currencyCode) : "Retail Value calculated by KITCO"}</strong></div><button className="ui-btn ui-btn-primary ui-btn-md" type="button" disabled={totalPairs === 0 || Boolean(validation) || pending !== null} onClick={() => void add()}>{pending === "save" ? "Saving…" : "Add to Current Order"}</button></section>
    {draft && <section className="commerce-current-order" aria-live="polite"><p className="commerce-eyebrow">Saved to Current Order</p><div className="commerce-order-line"><div><strong>{product.articleNo}</strong><span>{totalPairs} pairs</span></div><strong>{formatRetailValue(draft.retailValueMinor, draft.currencyCode)}</strong></div>
      ...
      {stage === "draft" && <button className="ui-btn ui-btn-primary ui-btn-md" type="button" disabled={!location} onClick={() => setStage("review")}>Review order</button>}
      {stage === "review" && <div className="commerce-review"><h2>Review Current Order</h2><p><strong>{totalPairs} pairs</strong> allocated to {locations.find((item) => item.id === location)?.name ?? location}.</p><p>Retail Value is based on current canonical MRP and will be recalculated when submitted.</p><button className="ui-btn ui-btn-primary ui-btn-md" type="button" disabled={pending !== null} onClick={() => void issueOtp()}>{pending === "otp" ? "Sending…" : "Send fresh order code"}</button></div>}
      {stage === "otp" && <div className="commerce-review"><h2>Verify this order</h2><label>Order verification code<input aria-label="Order verification code" value={otp} inputMode="numeric" maxLength={6} onChange={(event) => setOtp(event.target.value.replace(/\D/g, ""))} /></label><button className="ui-btn ui-btn-primary ui-btn-md" type="button" disabled={otp.length !== 6 || pending !== null} onClick={() => void submit()}>{pending === "submit" ? "Submitting…" : "Submit immutable order"}</button></div>}
