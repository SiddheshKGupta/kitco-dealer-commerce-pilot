...
import { saveDraft, submitOrder, type DraftResponse } from "./api";
...
export function DealerOrderJourney({ product, requestOrderOtp, onBack }: { product: CatalogueProduct; requestOrderOtp: (purpose: "ORDER_SUBMISSION") => Promise<string>; onBack?: () => void }) {
  ...
  const issueOtp = async () => { setError(""); try { setChallengeId(await requestOrderOtp("ORDER_SUBMISSION")); setStage("otp"); } catch (caught) { setError(caught instanceof Error ? caught.message : "The order code could not be sent. Try again."); } };
  const submit = async () => {
    if (otp.length !== 6 || !challengeId) return;
    setError("");
    try { const result = await submitOrder({ otpChallengeId: challengeId, otpDigest: otp, idempotencyKey: crypto.randomUUID() }); setVersion(result.order.version); setStage("submitted"); }
  ...
  return <main className="commerce-page commerce-pdp">
    ...
    <div className="commerce-pdp-grid"><div className="commerce-pdp-media">{product.mediaUrl ? <img src={product.mediaUrl} alt={`${product.articleNo} · ${product.colour}`} /> : <div className="commerce-placeholder" role="img" aria-label={`Image unavailable for ${product.articleNo}`}><b>{product.articleNo}</b><small>Image arriving soon</small></div>}</div>
      <section className="commerce-pdp-copy"><p className="commerce-eyebrow">{product.brand} · Exact colourway</p><h1>{product.articleNo}</h1><p className="commerce-colour">{product.colour}</p><p className="commerce-mrp">MRP {formatRetailValue(product.mrpMinor, product.currencyCode)}</p><div className="commerce-policy"><span>Minimum {product.offering.moqPairs} pairs</span><span>Multiple of {product.offering.orderMultiplePairs}</span></div>
    ...
    <section className={`commerce-order-tray ${totalPairs > 0 ? "is-active" : ""}`} aria-label="Current Order action"><div><span>{totalPairs} pairs selected</span><strong>{draft ? formatRetailValue(draft.retailValueMinor, draft.currencyCode) : "Retail Value calculated by KITCO"}</strong></div><button className="commerce-primary" type="button" disabled={totalPairs === 0 || Boolean(validation)} onClick={() => void add()}>Add to Current Order</button></section>
    {draft && <section className="commerce-current-order" aria-live="polite"><p className="commerce-eyebrow">Saved to Current Order</p><div className="commerce-order-line"><div><strong>{product.articleNo}</strong><span>{totalPairs} pairs</span></div><strong>{formatRetailValue(draft.retailValueMinor, draft.currencyCode)}</strong></div>
      <label>Ship-to location<select aria-label="Ship-to location" value={location} onChange={(event) => setLocation(event.target.value)}><option value="">Choose location</option><option>VLCO Main</option><option>Secondary store</option></select></label>
      {stage === "draft" && <button className="commerce-primary" type="button" disabled={!location} onClick={() => setStage("review")}>Review order</button>}
      {stage === "review" && <div className="commerce-review"><h2>Review Current Order</h2><p><strong>{totalPairs} pairs</strong> allocated to {location}.</p><p>Retail Value is based on current canonical MRP and will be recalculated when submitted.</p><button className="commerce-primary" type="button" onClick={() => void issueOtp()}>Send fresh order code</button></div>}
      {stage === "otp" && <div className="commerce-review"><h2>Verify this order</h2><label>Order verification code<input aria-label="Order verification code" value={otp} inputMode="numeric" maxLength={6} onChange={(event) => setOtp(event.target.value.replace(/\D/g, ""))} /></label><button className="commerce-primary" type="button" disabled={otp.length !== 6} onClick={() => void submit()}>Submit immutable order</button></div>}
