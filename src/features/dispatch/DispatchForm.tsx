...
export function DispatchForm({ orderId, allocation, request, onMessage }: { orderId: string; allocation: FulfilmentAllocation; request: (path: string, body: object) => Promise<unknown>; onMessage: (message: string) => void }) {
	...
	return <div className="control-action"><label htmlFor="dispatch-pairs">Dispatch pairs</label><input id="dispatch-pairs" type="number" min="1" value={pairs} onChange={(event) => setPairs(event.target.value)} /><button type="button" onClick={dispatch} disabled={!Number(pairs)}>Record dispatch</button></div>;
}
