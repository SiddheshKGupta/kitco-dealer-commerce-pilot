-- Fix: submit_kitco_order_v5 gated the ordering dealer on the v4 activation_status
-- column, never touched by the v5 account_state machine. verified-session.ts's
-- isActiveDealer() and SupabaseDealerGroups.isSelectableDealer() both already use
-- coalesce(account_state, activation_status) so a v5-onboarded dealer (account_state
-- set, activation_status left at its v4 default of UNACTIVATED) can sign in, browse
-- and check out -- then this RPC alone refused them with "dealer session is no
-- longer active", after a real order OTP had already been spent.
--
-- Confirmed live and in effect before this fix: 3 dealers with account_state =
-- 'ACTIVE' and activation_status = 'UNACTIVATED' (FIGURE8383, KLIFES7268,
-- TEST12201), all with credentials already issued.
--
-- Same precedence as the two TypeScript call sites, applied here rather than only
-- there: account_state is the source of truth once a dealer is on the v5 machine,
-- activation_status is the fallback for a dealer nothing has moved onto v5 yet.
...
begin
  ...
  if not exists (
    ...
    where id = p_dealer_id and organisation_id = p_organisation_id
      and coalesce(account_state, activation_status) = 'ACTIVE'
  ) then
