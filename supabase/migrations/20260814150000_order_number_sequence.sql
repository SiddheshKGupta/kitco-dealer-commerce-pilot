-- Task 1: order numbers a dealer can read out over the phone. Replaces the
-- meaningless uuid-substring order_number with KIT-YYMM-00001 -- year+month
-- prefix plus a per-organisation counter that resets each period. Existing
-- orders keep their current order_number; only new submissions use this.
