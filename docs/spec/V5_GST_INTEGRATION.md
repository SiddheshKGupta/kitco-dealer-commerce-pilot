...
| GSTIN checksum validation before calling the provider | Shape check only (15 chars, the standard state-code/PAN/entity layout). The mod-36 checksum is well-defined and could be added, but rejecting on it locally would block a valid edge case the provider would have accepted. |
